        ...
        return (
            <div className="filter-component row">
                <div className="filter-component col-xs-12">
                    <SearchFilter filterText={this.props.filters.filterText}
                                  onFilterTextInput={this.props.onFilterTextInput}/>
                </div>
                <div className="col-md-4 col-xs-12">
                    <DropdownFilter label="Department"
                                    onFilterInput={value => this.props.onFilterInput('department', value)}
                                    options={this.props.departments.map(department => ({value: department.id, label: department.name}))}
                                    myFilter={this.props.filters.department}/>
                </div>
                <div className="col-md-4 col-xs-12">
                    <DropdownFilter label="Role"
                                    onFilterInput={value => this.props.onFilterInput('role', value)}
                                    options={this.props.roles.map(role => ({value: role.id, label: role.name}))}
                                    myFilter={Convert.convertToDisplay(this.props.filters.role)}/>
                </div>
                <div className="col-md-4 col-xs-12">
                    <DropdownFilter label="Got Ticket"
                                    onFilterInput={(value) => this.props.onFilterInput('gotTicket', value)}
                                    options={[{value: true, label: 'Yes'}, {value: false, label: 'No'}]}
                                    myFilter={this.props.filters.gotTicket}/>
                </div>
                <div className="col-md-4 col-xs-12">
                    <DropdownFilter label="Production"
                                    onFilterInput={(value) => this.props.onFilterInput('isProduction', value)}
                                    options={[{value: 1, label: 'Yes'}, {value: 0, label: 'No'}]}
                                    myFilter={this.props.filters.isProduction}/>
                </div>
                <div className="col-md-4 col-xs-12">
