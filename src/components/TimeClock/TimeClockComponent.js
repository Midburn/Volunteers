                ...
                <ListGroup>
                    {shiftManagerModel.filteredShifts.sort().map(shift =>
                        <ListGroupItem key={shift.id} onClick={() => shiftManagerModel.currentShift = shift}>
                            ...
                            <span className="badge">{`${shift.volunteers ? Object.keys(shift.volunteers).filter(volunteerId => !!shift.volunteers[volunteerId].isCheckedIn).length : 0}/${_.size(shift.volunteers)}`}</span>
